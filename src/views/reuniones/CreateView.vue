        ...
        <template v-slot:main>
                ...
                <form class=" bg-white/80 p-6 rounded shadow-md" @submit.prevent="submit">
                    <div class="mb-4">
                        ...
                        <input v-model="form.titulo" id="titulo" class="w-full bg-gray-100 rounded p-2" type="text" placeholder="Título"
                            name="titulo" />
                    </div>
                    <div class="mb-4">
                        <label for="fecha" class="block text-gray-700 mb-2">Fecha</label>
                        <input v-model="form.fecha" id="fecha" class="w-full bg-gray-100 rounded p-2" type="date" placeholder="Fecha"
                            name="fecha" />
                    </div>
                    <div class="mb-4">
                        <label for="hora_inicio" class="block text-gray-700 mb-2">Hora de inicio</label>
                        <input v-model="form.hora_inicio" id="hora_inicio" class="w-full bg-gray-100 rounded p-2" type="time"
                    ...
                    <div class="mb-4">
                        ...
                        <input v-model="form.hora_finalizacion" id="hora_finalizacion" class="w-full bg-gray-100 rounded p-2" type="time"
                            placeholder="Hora de finalización" name="hora_finalizacion" />
                    </div>
                    <div class="mb-4">
                        <label for="lugar" class="block text-gray-700 mb-2">Lugar</label>
                        <input v-model="form.lugar" id="lugar" class="w-full bg-gray-100 rounded p-2" type="text" placeholder="Lugar"
                            name="lugar" />
                    </div>
                    <div class="mb-8">
...
const { storeReuniones } = useReunionesStore();
...
const submit = async () => {
    try {
        ...
        const response = await storeReuniones(form.value);
